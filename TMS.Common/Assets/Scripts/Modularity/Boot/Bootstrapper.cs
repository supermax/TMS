...
namespace TMS.Common.Modularity
{
	...
	public class Bootstrapper<T> : IBootstrapper<T>
	{
		...
		public IDictionary<ModuleInfo, IModule> LoadModuleCatalog(string catalogPath, params object[] args)
		{
			...
			var conv = res.ToDictionary<KeyValuePair<ModuleInfo, IModule<T>>, ModuleInfo, IModule>(module => module.Key,
				module => module.Value);
			return conv;
		}
#else
...
/// <param name="args">The args.</param>
		public async Task<IDictionary<ModuleInfo, IModule>> LoadModuleCatalog(string catalogPath, params object[] args)
		{
			...
			var conv =
 res.ToDictionary<KeyValuePair<ModuleInfo, IModule<T>>, ModuleInfo, IModule>(module => module.Key, module => module.Value);
			return conv;
		}
#endif
						...
						case ModulePriorityType.Critical:
							{
								throw new OperationCanceledException(
									string.Format(
										Resources.ErrMsg_ModuleLoading_Aborted,
...
/// <param name="api">The API.</param>
		...
		{
			var txt = UnityEngine.Resources.Load<TextAsset>(filePath);
...
/// <returns></returns>
	...
	{
		private static IEnumerable<Assembly> _assemblyCache;
