...
namespace Unity.IO.Compression
{
	...
	{
		private uint bitBuffer; // store the bits here, we can quickly shift in this buffer
		...
		{
			get { return end - start + AvailableBits / 8; }
		}
		...
		public bool EnsureBitsAvailable(int count)
		{
			...
			if (AvailableBits < count)
			{
				...
				bitBuffer |= (uint) buffer[start++] << AvailableBits;
				AvailableBits += 8;
		...
		public int GetBits(int count)
		{
			...
			var result = (int) (bitBuffer & GetBitMask(count));
			bitBuffer >>= count;
			AvailableBits -= count;
			return result;
		}
