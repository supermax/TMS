	...
	public class DeflateStream : Stream
	{
		...
		public DeflateStream(Stream stream, CompressionMode mode, bool leaveOpen)
		{
			...
			switch (_mode)
			{
				case CompressionMode.Decompress:
					...
					if (!BaseStream.CanRead) throw new ArgumentException(SR.GetString(SR.NotReadableStream), "stream");
				...
				case CompressionMode.Compress:
					...
					if (!BaseStream.CanWrite) throw new ArgumentException(SR.GetString(SR.NotWriteableStream), "stream");
		...
		public override bool CanRead
		{
			get
			{
				...
				return _mode == CompressionMode.Decompress && BaseStream.CanRead;
			}
		}
		...
		public override bool CanWrite
		{
			get
			{
				...
				return _mode == CompressionMode.Compress && BaseStream.CanWrite;
			}
		}
		...
		{
			if (writer != null) formatWriter = writer;
		}
		...
		public override int Read(byte[] array, int offset, int count)
		{
			...
			while (true)
			{
				...
				var bytes = BaseStream.Read(buffer, 0, buffer.Length);
				if (bytes == 0) break; //Do we want to throw an exception here?
		...
		private void WriteDeflaterOutput(bool isAsync)
		{
			...
			{
				var compressedBytes = deflater.GetDeflateOutput(buffer);
		...
		public override int EndRead(IAsyncResult asyncResult)
		{
			...
			if (previousException != null) throw previousException;
