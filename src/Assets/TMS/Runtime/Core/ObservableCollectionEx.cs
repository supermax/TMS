...
namespace TMS.Common.Core
{
	...
	[Serializable]
	public class ObservableCollectionEx<T> : Collection<T>, 
		INotifyCollectionChanged, INotifyPropertyChanged, IDisposable
	{
		...
		public ObservableCollectionEx(ICollection<T> list)
			: base((list != null) ? new List<T>(list.Count) : new List<T>())
		{
			...
			add
			{
				if (null == _notifyInfo)
				{
					if (null == PropertyChanged)
					{
						...
						_fireIndexerChanged = () => OnPropertyChanged(new PropertyChangedEventArgs(IndexerName));
					}
		...
		protected virtual void Dispose(bool reason)
		{
			...
			if (null == _notifyInfo || !_notifyInfo.HasEventArgs) return;
		...
		public void Sort(ListSortDirection dir, bool disableNotifications = false)
		{
			...
			OnCollectionChanged(new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset)); // TODO need to notify about old\new items ??
		}
	}
}