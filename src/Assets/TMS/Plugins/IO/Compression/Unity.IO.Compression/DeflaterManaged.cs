...
namespace Unity.IO.Compression
{
	internal class DeflaterManaged : IDeflater
	{
		...
		private readonly CopyEncoder copyEncoder;

		private readonly FastEncoder deflateEncoder;

		private readonly DeflateInput input;
		...
		int IDeflater.GetDeflateOutput(byte[] outputBuffer)
		{
			...
			switch (processingState)
			{
				case DeflaterState.NotStarted:
				{
					...
					var initialInputState = input.DumpState();
					var initialOutputState = output.DumpState();
				...
				case DeflaterState.SlowDownForIncompressible2:
				{
					...
					if (inputFromHistory.Count > 0) copyEncoder.GetBlock(inputFromHistory, output, false);
		...
		bool IDeflater.Finish(byte[] outputBuffer, out int bytesRead)
		{
			...
			if (processingState == DeflaterState.CompressThenCheck ||
			    processingState == DeflaterState.HandlingSmallData ||
			    processingState == DeflaterState.SlowDownForIncompressible1) deflateEncoder.GetBlockFooter(output);
