...
namespace Unity.IO.Compression
{
	...
	internal class GZipDecoder : IFileFormatReader
	{
			...
			while (true)
			{
				...
				switch (gzipHeaderSubstate)
				{
					case GzipHeaderState.ReadingID1:
						...
						if (bits != GZipConstants.ID1) throw new InvalidDataException(SR.GetString(SR.CorruptedGZipHeader));
					...
					case GzipHeaderState.ReadingID2:
						...
						if (bits != GZipConstants.ID2) throw new InvalidDataException(SR.GetString(SR.CorruptedGZipHeader));
					...
					case GzipHeaderState.ReadingCM:
						...
						if (bits != GZipConstants.Deflate) throw new InvalidDataException(SR.GetString(SR.UnknownCompressionMode));
		...
		public void UpdateWithBytesRead(byte[] buffer, int offset, int copied)
		{
			...
			if (n >= GZipConstants.FileLengthModulo) n %= GZipConstants.FileLengthModulo;
			actualStreamSizeModulo = n;
		}

		public void Validate()
		{
			if (expectedCrc32 != actualCrc32) throw new InvalidDataException(SR.GetString(SR.InvalidCRC));
