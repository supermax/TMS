...
namespace Unity.IO.Compression
{
	internal class FastEncoderWindow
	{
		...
		public void MoveWindows()
		{
			...
			{
				var val = lookup[i] - FastEncoderWindowSize;

				if (val <= 0) lookup[i] = 0;
				else lookup[i] = (ushort) val;
			}
			...
			{
				var val = (long) prev[i] - FastEncoderWindowSize;

				if (val <= 0) prev[i] = 0;
				else prev[i] = (ushort) val;
			}
		...
		private void InsertStrings(ref uint hash, int matchLen)
		{
			...
			if (bufEnd - bufPos <= matchLen) bufPos += matchLen - 1;
			else
				while (--matchLen > 0)
				{
		...
		internal bool GetNextSymbolOrMatch(Match match)
		{
			...
			else
			{
				...
				if (matchLen <= LazyMatchThreshold)
				{
					...
					if (search != 0)
					{
						...
						if (bufPos + nextMatchLen > bufEnd) nextMatchLen = bufEnd - bufPos;
					}
					else
					{
						nextMatchLen = 0;
			...
			if (bufPos == 2 * FastEncoderWindowSize) MoveWindows();
			return true;
		}
		...
		private int FindMatch(int search, out int matchPos, int searchDepth, int niceLength)
		{
			...
			Debug.Assert(RecalculateHash(search) == RecalculateHash(bufPos));
			...
			var earliest = bufPos - FastEncoderWindowSize;
			Debug.Assert(earliest >= 0, "bufPos is less than FastEncoderWindowSize!");

			var wantChar = window[bufPos];
			while (search > earliest)
			{
				// make sure all our hash links are valid
				Debug.Assert(RecalculateHash(search) == RecalculateHash(bufPos), "Corrupted hash link!");
			...
			if (bestMatch == 3 && matchPos >= FastEncoderMatch3DistThreshold) return 0;
		...
		private void VerifyHashes()
		{
			for (var i = 0; i < FastEncoderHashtableSize; i++)
			{
				...
				while (where != 0 && bufPos - where < FastEncoderWindowSize)
				{
					...
					if (bufPos - nextWhere >= FastEncoderWindowSize) break;
